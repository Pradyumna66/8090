...
for ((i=0; i<num_cases; i++)); do
    ...
    #if script_output=$(./run.sh "$trip_duration" "$miles_traveled" "$receipts_amount" 2>&1); then
     #   echo "RAW script_output: >>>$script_output<<<" >&2
      #  output=$(echo "$script_output" | tr -d '\r' | tr -d '\n' | tr -d '[:space:]')
       # echo "PARSED output: >>>$output<<<" >&2
